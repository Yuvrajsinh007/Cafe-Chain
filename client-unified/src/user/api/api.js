...
const apiClient = async (endpoint, data = {}, options = {}) => {
  ...
  try {
    const response = await axios({
      ...
      headers,
      withCredentials: options.withCredentials || false, // Pass this through if needed
    });
    ...
    throw errorMessage; // Throw just the message string for easier handling in components
  }
};
...
export const claimReward = (formData) =>
  apiClient("/rewards/claim", formData, {
    method: "POST",
    headers: { "Content-Type": "multipart/form-data" }, // Let axios/browser handle boundary if possible, but keeping logic
...
export const getActiveEvents = async () => {
  try {
    // ✅ CRITICAL: Updated fetch URL to use API_URL logic
